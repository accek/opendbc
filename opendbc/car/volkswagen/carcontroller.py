...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP, CP_SP, CP_AC):
    ...
    # acspilot: MQB forwards stock messages with modified fields (ACSPilot architecture); PQ/MLB keep
    # the upstream pack-from-scratch carcontroller. accek's pqcan/mlbcan are pack-model, so the
    # forwarding path is MQB-only.
    self.acspilot_forward = not (CP.flags & (VolkswagenFlags.PQ | VolkswagenFlags.MLB))
    ...
    # stock pack-model state (PQ/MLB)
    self.apply_torque_last = 0
    self.gra_acc_counter_last = None
    self.hca_mitigation = HCAMitigation(self.CCP)

    # acspilot forwarding state (MQB)
  ...
  def update(self, CC, CC_SP, CC_AC, CS, now_nanos):
    if self.acspilot_forward:
      return self.update_acspilot(CC, CC_SP, CC_AC, CS, now_nanos)
    return self.update_stock(CC, CC_SP, CC_AC, CS, now_nanos)

  def update_stock(self, CC, CC_SP, CC_AC, CS, now_nanos):
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.frame % self.CCP.ACC_CONTROL_STEP == 0:
        acc_control = self.CCS.acc_control_value(CS.out.cruiseState.available, CS.out.accFaulted, CC.longActive)  # ty: ignore[missing-argument]
    ...
    if self.frame % self.CCP.ACC_HUD_STEP == 0 and self.CP.openpilotLongitudinalControl:
      ...
      acc_hud_status = self.CCS.acc_hud_status_value(CS.out.cruiseState.available, CS.out.accFaulted, CC.longActive)  # ty: ignore[missing-argument]
  ...
  def update_acspilot(self, CC, CC_SP, CC_AC, CS, now_nanos):
    actuators = CC.actuators
    hud_control = CC.hudControl
    can_sends = []
    ...
    if self.frame % self.CCP.STEER_STEP == 0:
      ...
      # Don't command the EPS while stationary, or whenever the engine is stopped. This honors "don't steer
      # at standstill" and avoids poking the EPS when it may have dropped out after the stop-start system
      # shut the engine off (including while coasting), which can otherwise provoke a steering fault.
      steer_inhibited = CS.out.standstill or not CS.engine_running

      if CC.latActive and CS.eps_init_complete and not steer_inhibited:
        new_torque = int(round(actuators.torque * self.CCP.STEER_MAX))
        apply_torque = apply_driver_steer_torque_limits(new_torque, self.apply_torque_last, CS.out.steeringTorque, self.CCP)
